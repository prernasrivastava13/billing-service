...
public class OutputMediaServiceImpl implements OutputMediaService {
  ...
  @Resource OutputMediaRepository outputMediaRepository;
  @Resource CreditInformationServiceImpl creditInformationService;
  @Resource AccountApplicationMappingService accountApplicationMappingService;
  @Resource AccountApplicationUsageLimitsService accountApplicationUsageLimitsService;

  @Override
  public OutputMedia createOutputMedia(OutputMedia outputMedia) {
    CreditInformation creditInformation =
        creditInformationService.getCreditInformationByApiId(outputMedia.getApplicationId());
    ...
    if (createdOutputMedia != null) {
      AccountApplicationMapping accountApplicationMapping =
          accountApplicationMappingService.getAccountApplicationMappingByAccountIdAndApplicationId(
              createdOutputMedia.getAccountId(), createdOutputMedia.getApplicationId());
      if (accountApplicationMapping != null) {
        AccountApplicationUsageLimits accountApplicationUsageLimits =
            accountApplicationUsageLimitsService.getAccountApplicationUsageLimitsByMappingId(
                accountApplicationMapping.getId());
        String curentmonth = "" + Calendar.getInstance().get(Calendar.MONTH) + 1;
        if (accountApplicationUsageLimits != null
            && accountApplicationUsageLimits.getMonth() == curentmonth) {
          ...
          accountApplicationMappingService.updateAccountApplicationMapping(
              accountApplicationMapping);
        }
        accountApplicationUsageLimits.setMonth(curentmonth);
        accountApplicationUsageLimits
            .getAccountApplicationMapping()
            .setId(accountApplicationMapping.getId());
        accountApplicationUsageLimits.setCreditUsed(creditsConsumed);
      }
      log.error(
          "No account application mapping found for account id: "
              + createdOutputMedia.getAccountId()
              + " and applicationId: "
  ...
  public List<OutputMedia> getOutputMediaByMediaId(int mediaId) {
    return StreamSupport.stream(
            outputMediaRepository
                .findAll(OutputMediaPredicate.getOutputMediaByMediaId(mediaId))
                .spliterator(),
            false)
  ...
  public List<OutputMedia> getOutputMediaByAccountId(int accountId) {
    List<OutputMedia> outputMediaList = outputMediaRepository.getByAccountId(accountId);
    Predicate predicate = OutputMediaPredicate.getOutputMediaByAccountId(accountId);
    Iterable<OutputMedia> outputMediaList1 = outputMediaRepository.findAll(predicate);
    Spliterator<OutputMedia> splitList = outputMediaList1.spliterator();
    Stream<OutputMedia> listStream = StreamSupport.stream(splitList, false);
    List<OutputMedia> List = listStream.collect(Collectors.toList());
    return outputMediaList;
  }
  ...
  public List<OutputMedia> getOutputMediaBySize(int minSize, int maxSize) {
    return StreamSupport.stream(
            outputMediaRepository
                .findAll(OutputMediaPredicate.getOutputMediaBySize(minSize, maxSize))
                .spliterator(),
            false)
        .collect(Collectors.toList());
  }
  ...
  public List<OutputMedia> getOutputMediaByApi(String api) {
    return StreamSupport.stream(
            outputMediaRepository
                .findAll(OutputMediaPredicate.getOutputMediaByApi(api))
                .spliterator(),
            false)
        .collect(Collectors.toList());
  }
  ...
  public List<OutputMedia> getOutputMediaByCreditConsumed(
    ...
    return StreamSupport.stream(
            outputMediaRepository
                .findAll(
                    OutputMediaPredicate.getOutputMediaCreditConsumed(
                        minCreditsConsumed, maxCreditsConsumed))
                .spliterator(),
            false)
        .collect(Collectors.toList());
  }
  ...
  public List<OutputMedia> getOutputMediaByApplicationId(int applicationId) {
    return StreamSupport.stream(
            outputMediaRepository
                .findAll(OutputMediaPredicate.getOutputMediaApplicationId(applicationId))
                .spliterator(),
            false)
        .collect(Collectors.toList());
  }
